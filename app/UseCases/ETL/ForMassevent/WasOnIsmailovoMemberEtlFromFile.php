...
class WasOnIsmailovoMemberEtlFromFile extends ETLAbstract implements EtlFromFileInterface
{
    ...
    public function extract(): int
    {
        ...
        $count = 0;
        for($row = 3; $row <= $bottomRight['row']; $row++) {
            $draftName = $currentSheet->getCell('B' . $row);

            $draftPhone = $currentSheet->getCell('A' . $row);
